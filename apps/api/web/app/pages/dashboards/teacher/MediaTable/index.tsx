...
import { Media, mediaList } from "./fakeData";
...
export function MediaTable() {
  ...
  const [media, setMedia] = useState<Media[]>([...mediaList]);
  ...
  const table = useReactTable({
    ...
    meta: {
      deleteRow: (row) => {
        ...
        setMedia((old) =>
          old.filter((oldRow) => oldRow.media_id !== row.original.media_id),
        );
      },
      deleteRows: (rows) => {
        // Skip page index reset until after next rerender
        skipAutoResetPageIndex();
        const rowIds = rows.map((row) => row.original.media_id);
        setMedia((old) => old.filter((row) => !rowIds.includes(row.media_id)));
      },
    },
    filterFns: {
      fuzzy: fuzzyFilter,
